...
type IContainerOrchestrator interface {
	...
	CreateLocalAggregatorServing(aggregator *model.FlAggregator, configFiles map[string]string) error
	RemoveLocalAggregator(aggregator *model.FlAggregator) error
	RemoveLocalAggregatorServing(aggregator *model.FlAggregator) error
